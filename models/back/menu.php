...
class ModelMenu extends JModel{
	...
	function crearEnlace($tipo, $datos){
		...
		if($tipo == "AR"){
			...
			$enlace = "index.php?option=com_ztienda&task=categoria&categoria=$datos";
		}
		else if($tipo == "PT"){
			$enlace = "index.php?option=com_ztienda";
		}
		else if($tipo == "CA"){
	...
	function guardarNuevoPariente($data , $tipo='H'){
		...
		$query = sprintf( $query, 
						  'mainmenu', $data['title'], $data['title'], $this->crearEnlace($data['tipo'], $data['datos']), 'component',
						  1, 1, '*', $position + 1, $position + 2, 
						  $parent, $params, 1
					    );
		$db->setQuery($query);
		$result = $db->query();
