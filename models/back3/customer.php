...
class ModelCustomer extends JModel{
    /**
	 * Constructor
	 */
	...
	* Lista de tiquetes
	*/
	function listarTiquetes($filtro, $usuario, $estado, $inicio, $registros){	
		$db = JFactory::getDBO();
		...
		$query = sprintf( $query, "%". $filtro . "%", "%". $filtro . "%",  $user->id);
		echo $query;
	...
	function contarTiquetes($filtro, $usuario, $estado){
		$db = JFactory::getDBO();
		$user = JFactory::getUser();
		...
		$query = sprintf( $query, "%".$filtro."%", $user->id );
		$db->setQuery($query);
